...
from typing import Union, List, Dict, Any  # , Set, Callable, Tuple, NoReturn
import nltk
        ...
        for state in args:
            if ":" in state:
                ...
                for source, target, speaker in extraction_df.transitions(state):
                    ...
                    self.dialogue_flow.dynamic_transitions().append((natex, (source, target, speaker), score))
            else:
    ...
    def run(self, ngrams: Ngrams, vars: Dict[str, Any], args: List[Any]):
        for state in args:
            if ":" in state:
                ...
                enter = self.dialogue_flow.composite_dialogue_flow().state_settings(component, state).enter
            else:
    ...
    def run(self, ngrams: Ngrams, vars: Dict[str, Any], args: List[Any]):
        ...
        if vars["__goal__"] != "None":
            ...
            if self.dialogue_flow.composite_dialogue_flow():
                push_goal_state = self.dialogue_flow.composite_dialogue_flow().controller_name() + ":" + push_goal_state
...
class ExpandContractions(Macro):
    def __init__(self):
        ...
        self.contractions = {
            "".join([a.lower() for a in k if a.isalpha() or a == " "]): v.lower() for k, v in contractions.items()
        }
